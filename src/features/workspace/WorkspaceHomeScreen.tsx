  ...
  return (
    <View style={styles.section}>
      <View style={styles.headerRow}>
        <Text style={[styles.sectionTitle, { color: colors.text.primary }]}>{hm.sectionNotes}</Text>
      ...
      <View style={[styles.listCard, { backgroundColor: colors.surface.panel }]}>
        ...
        >
          <View style={[styles.iconBubble, styles.actionIconBubble, { backgroundColor: colors.accent.selectionBg }]}>
            {creatingNote ? (
              ...
              <Icon source="square-edit-outline" size={16} color={accent} />
            )}
          </View>
          <View style={styles.itemCopy}>
            <Text numberOfLines={1} style={[styles.itemTitle, { color: colors.text.primary }]}>{hm.createNote}</Text>
            <Text numberOfLines={1} style={[styles.itemSubtitle, { color: colors.text.tertiary }]}>
              {creatingNote ? hm.creatingNote : hm.createNoteHint}
            </Text>
          </View>
        ...
        {loading ? (
          ...
          <View style={styles.emptyRowCompact}>
            <Text style={[styles.emptyText, { color: colors.text.tertiary }]}>{hm.noRecentNotes}</Text>
          </View>
        ) : (
          notes.map((note) => {
            const title = resolveNoteListTitle(note, hm.untitled, readLocalNote(note.id));
            ...
            <Pressable key={note.id} style={styles.itemRow} onPress={() => onNotePress(note)}>
              <View style={[styles.iconBubble, { backgroundColor: colors.accent.selectionBg }]}>
                <Icon source={iconForNoteKind(note.kind)} size={16} color={accent} />
              </View>
              <View style={styles.itemCopy}>
