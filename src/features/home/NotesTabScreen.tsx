/**
 * NotesTabScreen — thin wrapper around NotesScreen adapted for tab context.
 *
 * Removes modal-dismiss behavior; the back action navigates to Home tab instead.
 */
...
import { SwipeableNoteCard, type SwipeAction } from '../notes/SwipeableNoteCard';
import { flushPendingNotes, queueNote } from '../notes/notes-sync';
import { captureIntentBadgeKey, parseCaptureIntent } from '../notes/capture-parser';
...
export function NotesTabScreen() {
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.surface.base, paddingTop: insets.top }]}>
      {/* Header */}
      <View style={styles.header}>
        <Text style={[styles.headerTitle, { color: colors.text.primary }]}>{pm.title}</Text>
        <IconButton
          ...
          {captureText.trim().length > 0 && (() => {
            ...
            return (
              <View style={[styles.intentBadge, { backgroundColor: colors.accent.selectionBg, borderColor: colors.border.subtle }]}>
                ...
                <Text style={{ fontSize: 11, color: colors.accent.primary }}>{pm[badgeKey]}</Text>
              </View>
            );
          })()}
...
const styles = StyleSheet.create({
  ...
  filterStrip: { flexGrow: 0, marginBottom: 6 },
  filterScroll: { flexDirection: 'row', gap: 6, paddingHorizontal: 16 },
  ...
  composerWrap: { paddingHorizontal: 10, paddingTop: 6, paddingBottom: 4, gap: 4 },
  intentBadge: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  composerShell: { borderWidth: 1, borderRadius: 22, overflow: 'hidden' },
  composerRow: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingHorizontal: 6,
  ...
  recordingBtn: { backgroundColor: '#EF4444', borderRadius: 18 },
  actionBackdrop: {
